...
impl Ferry {
    ...
    pub fn stabilize(&mut self) -> usize {
        ...
        loop {
            ...
            println!("Generation {}: {} occupied seats", generation, occupied_seats);
    ...
    fn count_occupied_neighbors(&self, cell_x: usize, cell_y: usize) -> u8 {
        ...
        for [dx, dy] in &NEIGHBORS {
            if let (Some(x), Some(y)) = (Self::add(cell_x, *dx), Self::add(cell_y, *dy)) {
                if let Some(SeatLayout::Seat(true)) = self.seats.get(y).and_then(|row| row.get(x)) {
